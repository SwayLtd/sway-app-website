...
function handleScroll() {
    ...
    if (window.innerWidth >= 1025) {
        const maxScroll = (slidesCount - 1) * window.innerHeight; // 4 * window.innerHeight
        ...
        const maxTranslate = 4 * 0.5 * window.innerWidth;
...
function handleWheel(event) {
    if (window.innerWidth >= 1025) {
        ...
        const threshold = 100; // <-- Ajustez cette valeur selon la sensibilité souhaitée

        // On cumule les deltas de la molette
